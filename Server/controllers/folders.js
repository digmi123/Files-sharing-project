...
module.exports.insertRootFolderIntoDB = async (req, res, next) => {
  try {
    ...
    query.on("result", function (result) {
      ...
      serverLogger.info(
        `root folder was created - folder id : ${result.insertId}`
      );
      return next();
    });
  } catch (err) {
...
const createTree = async (folderID) => {
  return [
    ...(await createFolderTree(folderID)),
    ...(await createFileTree(folderID)),
  ];
};

module.exports.findRootFolder = (req, res, next) => {
  console.log("~~~~~~~~~~~~~~~~~findRootFolder~~~~~~~~~~~~~~~~~");
  console.log(req.body);
  const { projectID } = req.body;
  const sql = "SELECT folder_id FROM projects WHERE id=(?);";
  console.log(db.format(sql, [projectID]));
